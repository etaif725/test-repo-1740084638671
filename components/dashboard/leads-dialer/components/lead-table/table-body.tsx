...
export function LeadTableBody({
  ...
  if (leads.length === 0) {
    return (
      <TableBody>
        <TableRow>
          <TableCell colSpan={Object.keys(FIELD_MAPPINGS).length + 1} className="h-24 text-center text-black dark:text-white bg-white dark:bg-zinc-950 hover:bg-zinc-200 dark:hover:bg-zinc-900 transition-colors">
        ...
        <TableRow key={lead.id}>
          ...
          {Object.keys(FIELD_MAPPINGS).map((field) => (
            <TableCell
              ...
              className="p-0"
            >
              <div className="px-4 py-2 min-h-[2.5rem] flex items-center text-black dark:text-white bg-white dark:bg-zinc-950 hover:bg-zinc-200 dark:hover:bg-zinc-900 transition-colors">
                <CellRenderer
                  key={field}
                  lead={lead}
